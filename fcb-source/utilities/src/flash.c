...
FlashErrorStatus ReadCalibrationValuesFromFlash(
	...
	status = ReadSettingsFromFlash((uint8_t*) receiverCalibrationValues,
			sizeof(Receiver_CalibrationValues_TypeDef),
			FLASH_RECEIVER_CALIBRATION_PAGE,
			FLASH_RECEIVER_CALIBRATION_DATA_OFFSET);
...
FlashErrorStatus WriteCalibrationValuesToFlash(
	...
	status = WriteSettingsToFlash((uint8_t*) receiverCalibrationValues,
			sizeof(Receiver_CalibrationValues_TypeDef),
			FLASH_RECEIVER_CALIBRATION_PAGE,
			FLASH_RECEIVER_CALIBRATION_DATA_OFFSET);
...
static FlashErrorStatus WriteSettingsToFlash(const uint8_t* writeSettingsData,
	...
	if (!IsValidSettingsPage(settingsPageNbr)
			|| !IsValidPageSize(settingsPageOffset,
					writeSettingsDataSize + FLASH_WORD_BYTE_SIZE))
		return FLASH_ERROR;
	...
	memcpy(
			&tmpPage[FLASH_RECEIVER_CALIBRATION_DATA_OFFSET
					+ FLASH_WORD_BYTE_SIZE], writeSettingsData,
...
static FlashErrorStatus ReadSettingsFromFlash(uint8_t* readSettingsData,
	...
	if (!IsValidSettingsPage(settingsPageNbr)
			|| !IsValidPageSize(settingsPageOffset,
					readSettingsDataSize + FLASH_WORD_BYTE_SIZE))
		return FLASH_ERROR;
	...
	if (!ReadFlashBytes(tmpSettings,
			GetFlashPageOffsetAddress(settingsPageNbr,
					settingsPageOffset + FLASH_WORD_BYTE_SIZE),
			readSettingsDataSize))
		return FLASH_ERROR;

	/* Calculate CRC of loaded settings bytes */
	uint32_t CRC_Calculated = CalculateCRC(tmpSettings, readSettingsDataSize);
...
static FlashErrorStatus WriteFlashPage(const uint32_t* writeData,
	...
	while (i < FLASH_PAGE_SIZE / FLASH_WORD_BYTE_SIZE && HALStatus == HAL_OK
			&& IsValidFlashAddress(address)) {
		if (writeData[i] != ReadFlashWord(pageNbr, i)) // To prevent unnecessary writing (if byte values should == 0xFF anyway)
			HALStatus = HAL_FLASH_Program(TYPEPROGRAM_WORD, address,
					writeData[i]); // Word size is 32 bits/4 bytes => 1 page = 2048 bytes = 512 words
 ...
 */
static FlashErrorStatus IsValidFlashAddress(const uint32_t address) {
	return (address >= FLASH_BASE_ADDR
