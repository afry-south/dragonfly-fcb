...
size_t xCommandStringLength;
	...
	if( pxCommand == NULL )
	{
		...
		for( pxCommand = &xRegisteredCommands; pxCommand != NULL; pxCommand = pxCommand->pxNext )
		{
			...
			a parameter. */
			/* EDIT: Added '\r' and '\n' to the original CLI code as end of command string characters */
			if( ( pcCommandInput[ xCommandStringLength ] == ' ' ) || ( pcCommandInput[ xCommandStringLength ] == 0x00 ) || ( pcCommandInput[ xCommandStringLength ] == '\r' ) || ( pcCommandInput[ xCommandStringLength ] == '\n' ) )
			{
	...
	else
	{
		...
		snprintf( ( char * ) pcWriteBuffer,
		    xWriteBufferLen,
		    ( const char * const ) "Command \"%s\" not recognised.  Enter 'help' to view a list of available commands.\r\n\r\n",
		    pcCommandInput);
		xReturn = pdFALSE;
	}
